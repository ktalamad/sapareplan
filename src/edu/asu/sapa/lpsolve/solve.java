/* $Header: /var/lib/cvsroot/SapaReplan-MURI/src/edu/asu/sapa/lpsolve/solve.java,v 1.1 2009/02/19 18:45:11 bentonj Exp $ */
/* $Log: solve.java,v $
...
/* *** empty log message ***
/*
/* Revision 1.1.6.3  2008/04/27 12:47:50  will
/* *** empty log message ***
/*
/* Revision 1.1.6.2  2008/04/26 03:14:44  will
/* *** empty log message ***
/*
/* Revision 1.1.6.1  2008/04/25 11:03:41  will
/* Gigantic Refactoring complete.
/*
...
/* *** empty log message ***
/*
/* Revision 1.4  2006/02/07 20:18:17  bentonj
/* now using JNI methods to call external lp solver.
/*
 /* Revision 1.3  2006/01/30 02:45:19  minh
 /* *** empty log message ***
 /*
 /* Revision 1.2  2005/12/29 22:48:08  minh
 /* *** empty log message ***
 /*
 ...
 # change implements isConstant into implements lp.constant
 #
...
class rside /* contains relational op and rhs value */
{
	double value;
...
public class solve implements constant {
	...
	void inc_col_space(lprec lp) {
		if (lp.columns >= lp.columns_alloc) {
			...
			lp.orig_lowbo = new double[lp.sum_alloc + 1];
			System
					.arraycopy(double_ptr, 0, lp.orig_lowbo, 0,
							double_ptr.length);
			...
			lp.best_solution = new double[lp.sum_alloc + 1];
			System.arraycopy(double_ptr, 0, lp.best_solution, 0,
					double_ptr.length);
	...
	public void str_set_obj_fn(lprec lp, String row) {
		...
		if (i < lp.columns) {
			System.err
					.println("Not enough numbers in the string in str_set_obj_fn");
			System.exit(FAIL);
		}
		set_obj_fn(lp, arow);
	}

	public void add_constraint(lprec lp, double[] row, short constr_type,
			double rh) {
		matrec[] newmat;
		int i, j;
		int elmnr;
	...
	public void add_lag_con(lprec lp, double[] row, short con_type, double rhs) {
		...
		if (lp.nr_lagrange == 1) {
			...
			lp.lag_con_type = new short[lp.nr_lagrange];
			System
					.arraycopy(short_ptr, 0, lp.lag_con_type, 0,
							short_ptr.length);
		}
	...
	public void set_rh(lprec lp, int row, double value) {
		...
		{
			System.err
					.println("Warning: attempt to set RHS of objective function, ignored");
			return;
		}
		if (lp.scaling_used != FALSE)
			if (lp.ch_sign[row] != FALSE)
	...
	short is_feasible(lprec lp, double[] values) {
		...
		for (i = 1; i <= lp.columns; i++)
			for (elmnr = lp.col_end[i - 1]; elmnr < lp.col_end[i]; elmnr++)
				this_rhs[lp.mat[elmnr].row_nr] += lp.mat[elmnr].value
						* values[i];
		for (i = 1; i <= lp.rows; i++) {
	...
	/**
	 * BM: Newly added function by Minh B. Do to get the final solution in the
	 * form of array list (values of all variables) so we can interprete to get
	 * the final plan.
	 */
	...
	public void print_solution(lprec lp) {
		...
		for (i = 1; i <= lp.columns; i++)
			if (lp.names_used != FALSE)
				System.out.println(lp.col_name[i] + " "
						+ lp.best_solution[lp.rows + i]);
			else
		...
		if ((lp.verbose != FALSE || lp.print_duals != FALSE)) {
			if (lp.max_level != 1)
				System.out
						.print("These are the duals from the node that gave the optimal solution.\n");
			else
				System.out.print("\nDual values:\n");
			for (i = 1; i <= lp.rows; i++)
				if (lp.names_used != FALSE)
	...
	/*
	 * void write_MPS(lprec lp, FILE *output) { int i, j, marker; double
	 * *column;
	 * 
	 * 
	 * MALLOC(column, lp.rows+1, double); marker=0; output.print( "NAME %s\n",
	 * lp.lp_name); output.print( "ROWS\n"); for(i = 0; i <= lp.rows; i++) {
	 * if(i==0) output.print( " N "); else if(lp.orig_upbo[i]==lp.infinite)
	 * if(lp.ch_sign[i]) output.print( " G "); else output.print( " L "); else
	 * output.print( " E "); if (lp.names_used != FALSE) output.print( "%s\n",
	 * lp.row_name[i]); else output.print( "r_%d\n", i); }
	 * 
	 * output.print( "COLUMNS\n"); j = 0; for(i = 1; i <= lp.columns; i++) {
	 * if((lp.must_be_int[i+lp.rows]) && (marker % 2)==0) { output.print( "
	 * MARK%04d 'MARKER' 'INTORG'\n", marker); marker++; }
	 * if((!lp.must_be_int[i+lp.rows]) && (marker % 2)==1) { output.print( "
	 * MARK%04d 'MARKER' 'INTEND'\n", marker); marker++; } get_column(lp, i,
	 * column); j=0; if(lp.maximise) { if(column[j] != 0) { if (lp.names_used !=
	 * FALSE) output.print( " %-8s %-8s %g\n", lp.col_name[i], lp.row_name[j],
	 * -column[j]); else output.print( " var_%-4d r_%-6d %g\n", i, j,
	 * -column[j]); } } else { if(column[j] != 0) { if (lp.names_used != FALSE)
	 * output.print( " %-8s %-8s %g\n", lp.col_name[i], lp.row_name[j],
	 * column[j]); else output.print( " var_%-4d r_%-6d %g\n", i, j, column[j]); } }
	 * for(j=1; j <= lp.rows; j++) if(column[j] != 0) { if (lp.names_used !=
	 * FALSE) output.print( " %-8s %-8s %g\n", lp.col_name[i], lp.row_name[j],
	 * column[j]); else output.print( " var_%-4d r_%-6d %g\n", i, j, column[j]); } }
	 * if((marker % 2) ==1) { output.print( " MARK%04d 'MARKER' 'INTEND'\n",
	 * marker); marker++; }
	 * 
	 * output.print( "RHS\n"); for(i = 1; i <= lp.rows; i++) { if(lp.ch_sign[i]) {
	 * if (lp.names_used != FALSE) output.print( " RHS %-8s %g\n",
	 * lp.row_name[i], (double)-lp.orig_rh[i]); else output.print( " RHS r_%-6d
	 * %g\n", i, (double)-lp.orig_rh[i]); } else { if (lp.names_used != FALSE)
	 * output.print( " RHS %-8s %g\n", lp.row_name[i], (double)lp.orig_rh[i]);
	 * else output.print( " RHS r_%-6d %g\n", i, (double)lp.orig_rh[i]); } }
	 * 
	 * output.print( "BOUNDS\n"); if (lp.names_used != FALSE) for(i = lp.rows +
	 * 1; i <= lp.sum; i++) { if(lp.orig_upbo[i] < lp.infinite) output.print( "
	 * UP BND %-8s %g\n", lp.col_name[i- lp.rows], (double)lp.orig_upbo[i]);
	 * if(lp.orig_lowbo[i] != 0) output.print( " LO BND %-8s %g\n",
	 * lp.col_name[i- lp.rows], (double)lp.orig_lowbo[i]); } else for(i =
	 * lp.rows + 1; i <= lp.sum; i++) { if(lp.orig_upbo[i] < lp.infinite)
	 * output.print( " UP BND var_%-4d %g\n", i - lp.rows,
	 * (double)lp.orig_upbo[i]); if(lp.orig_lowbo[i] != 0) output.print( " LO
	...
	public void print_duals(lprec lp) {
		...
		for (i = 1; i <= lp.rows; i++)
			if (lp.names_used != FALSE)
				System.out.println(lp.row_name[i] + " [" + i + "] "
						+ lp.duals[i]);
			else
	...
	public void print_scales(lprec lp) {
		...
		if (lp.scaling_used != FALSE) {
			for (i = 0; i <= lp.rows; i++)
				System.out
						.println("Row[" + i + "]    scaled at " + lp.scale[i]);
			for (i = 1; i <= lp.columns; i++)
				System.out.println("Column[" + i + "] scaled at "
						+ lp.scale[lp.rows + i]);
		}
	}
		...
		for (i = 1; i <= lp.columns; i++)
			if (colnum[i] == 0)
				if (lp.names_used != FALSE)
					...
					System.err.print("Warning: Variable " + i
							+ " not used in any constaints\n");
		lp.valid = TRUE;
		return (TRUE);
	}
	...
	private void rhsmincol(double theta, int row_nr, int varin) {
		...
		if (row_nr > Rows + 1) {
			System.err.print("Error: rhsmincol called with row_nr: " + row_nr
					+ ", rows: " + Rows + "\n");
	...
	void invert() {
		...
		if (Lp.print_at_invert != FALSE)
			System.out.print("Start Invert iter " + Lp.iter + " eta_size "
					+ Eta_size + " rhs[0] " + (-Rhs[0]) + " \n");
		...
		for (j = 1; j <= Columns; j++)
			if (fcol[j - 1] != FALSE) {
				...
				row_nr = 1;
				while ((row_nr <= Rows)
						&& (frow[row_nr] == FALSE || pcol[row_nr] == FALSE))
					row_nr++; /*
								 * this sometimes sets row_nr to Rows + 1 and
								 * makes rhsmincol crash. Solved in 2.0? MB
								 */
				if (row_nr == Rows + 1)
					System.err
							.print("lp_solve.solve.invert(): Inverting failed\n");
				frow[row_nr] = FALSE;
		...
		if (Lp.print_at_invert != FALSE)
			System.out.print("End Invert                eta_size " + Eta_size
					+ " rhs[0] " + (-Rhs[0]) + "\n");
	...
	private short colprim(Ref colnr, short minit, double[] drow) {
		...
		/*
		 * System.out.println("dpiv = " + dpiv); System.out.println("drow[] at
		 * colprim"); for(i = 1; i <= Sum; i++) // DEBUG {
		 * System.out.print(drow[i] + " "); } System.out.println();
		 */
		for (i = 1; i <= Sum; i++)
			if (Basis[i] == FALSE)
				if (Upbo[i] > 0) {
		...
		if (Lp.trace != FALSE)
			if (colnr.value > 0)
				System.err.print("col_prim:" + colnr.value + ", reduced costs: "
						+ dpiv + "\n");
			else
				System.err
						.print("col_prim: no negative reduced costs found, optimality!\n");
		if (colnr.value == 0) {
			Doiter = FALSE;
			DoInvert = FALSE;
			Status = OPTIMAL;
	...
	private short rowprim(int colnr, Ref row_nr, Ref theta, double[] pcol) {
		...
		if (theta.value < 0) {
			System.err.println("Warning: Numerical instability, qout = "
					+ theta.value);
		...
		if (row_nr.value > 0)
			Doiter = TRUE;
		if (Lp.trace != FALSE)
			System.out.println("row_prim:" + row_nr.value + ", pivot fluent:"
		...
		if (Lp.trace != FALSE) {
			if (row_nr.value > 0) {
				...
				if (Upbo[Bas[(int) row_nr.value]] < Infinite)
					System.out
							.println("               upper bound of basis variable:    "
	...
	private short coldual(int row_nr, Ref colnr, short minit, double[] prow,
			double[] drow) {
		int i, j, r, varnr;
		...
		if (Lp.trace != FALSE)
			System.out.println("col_dual:" + colnr.value + ", pivot fluent:  "
	...
	private void iteration(int row_nr, int varin, Ref theta, double up,
			Ref minit, Ref low, short primal, double[] pcol) {
		int i, k, varout;
		double f;
		double pivot;
		...
		if (Lp.trace != FALSE) {
			...
			if (minit.value != 0) {
				if (Lower[varin] == FALSE)
					System.out.print("Iteration:" + Lp.iter + ", variable"
							+ varin + " changed from 0 to its upper bound of "
							+ Upbo[varin] + "\n");
				else
					System.out.print("Iteration:" + Lp.iter + ", variable"
							+ varin + " changed its upper bound of "
							+ Upbo[varin] + " to 0\n");
			} else
			...
			if (primal == 0) {
				...
				for (i = 1; i <= Rows; i++)
					if (Rhs[i] < 0)
						...
						f += Rhs[i] - Upbo[Bas[i]];
				System.out.println("feasibility gap of this basis:"
						+ (double) f);
			} else
				System.out
						.println("objective function value of this feasible basis: "
								+ Rhs[0]);
		}
	} /* iteration */
	...
	private int solvelp() {
		...
		while (i != Rows && primal != FALSE) {
			i++;
			primal = (Rhs[i] >= 0 && Rhs[i] <= Upbo[Bas[i]]) ? (short) 1
					: (short) 0;
		}
		if (Lp.trace != FALSE) {
			if (primal != FALSE)
		...
		while (Status == RUNNING) {
			...
			} else /* not primal */
			{
				...
				if (row_nr > 0) {
					...
					if (flag != FALSE) {
						...
						if (Pcol[row_nr] == 0) {
							System.err
									.println("An attempt was made to divide by zero (Pcol["
											+ row_nr + "])");
							System.err
							...
							if (f > 0) {
								...
								if (theta <= Upbo[colnr])
									Lower[Bas[row_nr]] = (Lower[Bas[row_nr]] == FALSE) ? (short) 1
											: (short) 0;
							} else
								/* f <= 0 */
								theta = Rhs[row_nr] / (double) Pcol[row_nr];
			...
			if (Doiter != FALSE) {
				...
				iteration(row_nr, colnr, ref1, Upbo[colnr], ref2, ref3, primal,
						Pcol);
				theta = ref1.value;
	...
	private void calculate_duals() {
		...
		/*
		 * When the slack is at its upper bound, change the sign. Can this
		 * happen?
		 */
		for (i = 1; i <= Rows; i++) {
			if (Lp.basis[i] != FALSE)
	...
	private int milpsolve(double[] upbo, double[] lowbo, short[] sbasis,
			short[] slower, int[] sbas) {
		...
		if (failure == OPTIMAL) /* there is a solution */
		{
			...
			/*
			 * if this solution is worse than the best sofar, this branch must
			 * die
			 */
			if (Maximise != FALSE)
			...
			if (is_worse != FALSE) {
				if (Lp.verbose != FALSE)
					System.out.println("level" + Level + " OPT NOB value "
							+ Solution[0] + " bound " + Best_solution[0]);
			...
			if (Lp.bb_rule == FIRST_NI) {
				...
				while (i <= Sum && notint == 0) {
					if (Must_be_int[i] != FALSE && is_int(Solution[i]) == FALSE)
						...
						{
							System.err.println("Warning: integer var "
									+ (i - Rows) + " is already fixed at "
									+ lowbo[i] + ", but has non-integer value "
									+ Solution[i]);

							System.err
									.println("Perhaps the -e option should be used");
						} else
							notint = i;
					i++;
				}
			...
			if (Lp.bb_rule == RAND_NI) {
				...
				if (nr_not_int == 0)
					...
					while (select_not_int > 0) {
						if (Must_be_int[i] != FALSE
								&& is_int(Solution[i]) == FALSE)
							select_not_int--;
						i++;
					}
					notint = i - 1;
			...
			if (Lp.verbose == TRUE)
				if (notint != FALSE)
					System.out.println("level " + Level + " OPT     value "
							+ Solution[0]);
				else
					System.out.println("level " + Level + " OPT INT value "
							+ Solution[0]);
			...
			if (notint != FALSE) /* there is at least one value not yet int */
			{
				...
				if (Lp.names_used != FALSE)
					debug_print("not enough ints. Selecting var "
							+ Lp.col_name[notint - Rows] + ", val: "
							+ Solution[notint]);
				else
				...
				if (Floor_first != FALSE) {
					...
					if (new_bound < lowbo[notint]) {
						debug_print("New upper bound value " + new_bound
								+ " conflicts with old lower bound "
								+ lowbo[notint] + "\n");
						resone = MILP_FAIL;
					} else /* bound feasible */
					{
						...
						Lp.eta_valid = FALSE;
						resone = milpsolve(new_upbo, lowbo, new_basis,
								new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
					new_bound += 1;
					if (new_bound > upbo[notint]) {
						debug_print("New lower bound value " + new_bound
								+ " conflicts with old upper bound "
								+ upbo[notint] + "\n");
						restwo = MILP_FAIL;
					} else /* bound feasible */
					{
						...
						Lp.eta_valid = FALSE;
						restwo = milpsolve(upbo, new_lowbo, new_basis,
								new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
				} else /* take ceiling first */
				{
					...
					} else /* bound feasible */
					{
						...
						debug_print_bounds(upbo, new_lowbo);
						Lp.eta_valid = FALSE;
						resone = milpsolve(upbo, new_lowbo, new_basis,
								new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
					new_bound -= 1;
					...
					} else /* bound feasible */
					{
						...
						debug_print_bounds(new_upbo, lowbo);
						Lp.eta_valid = FALSE;
						restwo = milpsolve(new_upbo, lowbo, new_basis,
								new_lower, new_bas);
						Lp.eta_valid = FALSE;
					}
				}
			...
			} else /* all required values are int */
			{
				...
				{
					if (Lp.debug != FALSE
							|| (Lp.verbose != FALSE && Lp.print_sol == FALSE))
						System.out.print("*** new best solution: old: "
								+ Best_solution[0] + ", new: " + Solution[0]
								+ " ***\n");
					...
					if (Lp.break_at_int != FALSE) {
						if (Maximise != FALSE
								&& (Best_solution[0] > Lp.break_value))
							Break_bb = TRUE;
						if (Maximise == FALSE
								&& (Best_solution[0] < Lp.break_value))
							Break_bb = TRUE;
	...
	public int lag_solve(lprec lp, double start_bound, int num_iter,
			short verbose) {
		...
		while (status == RUNNING) {
			...
			if (verbose != FALSE) {
				System.out.println("Zub: " + Zub + " Zlb: " + Zlb + " Step: "
						+ Step + " pie: " + pie + " Feas " + OrigFeas);
				for (i = 0; i < lp.nr_lagrange; i++)
					System.out.println(i + " SubGrad " + SubGrad[i]
							+ " lambda " + lp.lambda[i]);
			}
			...
			while (same_basis != FALSE && i < lp.sum) {
				same_basis = (old_lower[i] == lp.lower[i]) ? (short) 1
						: (short) 0;
				i++;
			}
			if (same_basis == FALSE) {
			...
			if (verbose != FALSE)
				System.out.println("result: " + result + "  same basis: "
						+ same_basis);

...
public class OCPlan {
	...
	int lmIndex, rmIndex; // starting index of logical mutex and resource
	// mutex in each array
	int clCount, lmCount, rmCount;
	int[] sa, ea, obj; // Start/end action & object related with the relation
	boolean[] st, et; // Start/end actions's time points connected by the
	// logical relation
	// TRUE: start time point; FALSE: end time point
	float[] sDur; // Temporal duration separated two act's starting times by
	// the logical relation

	ArrayList<TemporalOrder>[] stn; // Simple Temporal Network to
	// store the temporal relations
	// between actions's starting times.

	int planSize;
	// ArrayList actSigs, aDurs;
	float[] est; // Action's earliest starting time according to the logical
	// relations
	...
	public OCPlan(int numAct, int numCL, int numLM, int numRM) {
		...
		for (int i = 0; i < planSize; i++)
			stn[i] = new ArrayList<TemporalOrder>();
	}
	 ...
	 */
	public void addLogOrder(int type, int index, int s, int e, boolean tp1,
			boolean tp2, int o, float dur) {
		int i;
	...
	public void addLogOrder(int type, int s, int e, boolean tp1, boolean tp2,
			int o, float dur) {
		int i;
		...
		case CL: // causal link
			...
			if (clCount > lmIndex) {
				System.out
						.println("OCPlan.addLogOrder(): Add more causal link that initialized.");
				return;
			}
			break;
		case LM: // logical mutex
			...
			if (lmCount > (rmIndex - lmIndex)) {
				System.out
						.println("OCPlan.addLogOrder(): Add more logical mutex that initialized.");
				return;
			}
			break;
		default:
	...
	public boolean subsummedBy(OCPlan ocPlan) {
		...
		for (i = 0; i < lmIndex; i++) {
			if (!ocPlan.containsLogOrder(CL, sa[i], ea[i], obj[i])) {
				equal = false;
				System.out.println(">>CL: (" + sa[i] + "--" + obj[i] + "-->"
						+ ea[i] + ")");
			}
		}
		...
		for (i = lmIndex; i < rmIndex; i++) {
			if (!ocPlan.containsLogOrder(LM, sa[i], ea[i], obj[i])) {
				if ((sa[i] != 0) && (ea[i] != planSize - 1)) // Disregard
					// mutex
					// involving
					// A_init & A_goal
					equal = false;
				System.out.println(">>LM: (" + sa[i] + "--" + obj[i] + "-->"
						+ ea[i] + ")");
			}
		}
		...
		for (i = rmIndex; i < size; i++) {
			if (!ocPlan.containsLogOrder(RM, sa[i], ea[i], obj[i])) {
				equal = false;
				System.out.println(">>RM: (" + sa[i] + "--" + obj[i] + "-->"
						+ ea[i] + ")");
			}
		}
	...
	public boolean equalsEST(OCPlan ocPlan) {
		...
		for (int i = 0; i < planSize; i++) {
			...
			if ((dif > EPS) || (-dif > EPS)) {
				equal = false;
				System.out.println(">> A" + i + ": st1 = " + est[i]
						+ " | st2 = " + ocPlan.getEST(i));
			}
		}
	...
	/**
	 * Function to sort the temporal orderings in an o.c. plan to find the
	 * smallest starting time for each action in the plan. Note: Not very
	 * efficient algorithm now, just for the purpose of easy to write.
	 */
	public void sort() {
		int i, j, k, numProcessed = 0;
		ArrayList[] tempStn = new ArrayList[planSize];
		TemporalOrder to;
	...
	/**
	 * Function to return the o.c plan in the form of (i) logical orderings
	 * between actions; (ii) temporal orderings (stn); (iii) earliest starting
	 * times
	 */
